...
object MovieList {
    ...
    public fun setupDynMovies(message: Result<List<String>>): List<Movie> {
        ...
        val tepList = this.convertToArray(message)
        val list = tepList.indices.map {
            buildMovieInfo(
                tepList[it],
                videoUrl[it],
                cardImageUrl[it]
    ...
    public fun convertToArray(message: Result<List<String>>): Array<String> {
        return message
            .getOrElse { emptyList() }  // Handle failure case (returns empty list)
            .toTypedArray()              // Convert List<String> to Array<String>
    }
